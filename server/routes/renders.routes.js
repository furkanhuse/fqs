...
router.get(
  "/:id([a-zA-Z0-9_-]{4,})",  // Matches typical short codes, e.g., abc123 or xY_Z
  locals.noLayout,
asyncHandler(async (req, res) => {
  const link = await Link.findOne({ where: { /* your condition here */ } });
  if (!link) return res.status(404).send('Not found');
  res.render("interstitial",{link:link.target}); // or whatever the target URL field is
})
);
